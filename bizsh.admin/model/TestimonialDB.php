...
class TestimonialDB
{
    ...
    public function getAllTestimonials() {
        $db = $this->connectDB();
        $sql = "SELECT m.msme_id, msme_name, m.msme_logo, t.message, CONCAT(e.e_lname,', ' , e.e_fname, ' ', e.e_mname) AS entrepname, e.e_photo, t.status , t.testi_id
            FROM msme m, entreprenuer e, testimonials t
            WHERE (e.e_id = m.e_id AND t.msme_id = m.msme_id)";
        $result = false;
        try {
            $stmt = $db->prepare($sql);
            $stmt->execute();
    ...
    public function updateTestimonial($data) {
        $db = $this->connectDB();
        $sql = "UPDATE testimonials SET status = '".$data['stat']."' 
        WHERE testi_id = '".$data['testi_id']."' ";
        $result = false;
        try {
            $stmt = $db->prepare($sql);
            $result = $stmt->execute();
